# ===================================================================
#           CSC - COMPLETE EMPTY EXAM FILE GENERATOR
# ===================================================================
# This script creates an organized folder structure and a blank .json 
# file for EVERY exam listed in the master data source below.
# -------------------------------------------------------------------
...
$baseOutputDirectory = "C:\Exams\CSC_Data"


# --- STEP 2: DEFINE THE MASTER DATA SOURCE (ALL SUBJECTS) ---
# All your exam lists have been consolidated here with their subject directory.
...
foreach ($exam in $allExams) {
    # Create the path for the subject's folder
    $subjectPath = Join-Path -Path $baseOutputDirectory -ChildPath $exam.subjectDirectory

    # Create the subject folder IF it doesn't already exist
    ...
    # Create the full path for the final JSON file
    $jsonFileName = $exam.fileName + ".json"
    $jsonFilePath = Join-Path -Path $subjectPath -ChildPath $jsonFileName

    # Create a new, empty file. -Force will overwrite if it exists.
    # An empty JSON file is just '{}'
